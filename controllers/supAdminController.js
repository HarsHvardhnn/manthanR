...
const submitReport = async (req, res) => {
  try {
    ...
    if (!user || !message) {
      return res
        .status(400)
        .json({ error: "User and message are required fields." });
    }
    ...
    const report = await supAdminModel.create({
      user: userId,
      message: message,
      admin: admin,
    });
  ...
  try {
    ...
    for (const supAdminUser of supAdminUsers) {
      const userModelData = await userModel
        .findById(supAdminUser.user, "username email score contactNumber ")
        .lean();
...
const getAdminReportedUsers = async (req, res) => {
  try {
    ...
    for (const report of reports) {
      const userModelData = await userModel
        .findById(report.user, "username email score contactNumber")
        .lean();
