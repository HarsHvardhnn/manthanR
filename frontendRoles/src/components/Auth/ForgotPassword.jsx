...
const ForgotPassword = () => {
  ...
  const sendOtp = (values) => {
    ...
    axios.post('http://localhost:4000/v1/sendOtp', { email: values.email }).then((res) => {
      if (res.status === 200) {
  ...
  const onSubmit = (values, { setSubmitting }) => {
    axios.post('http://localhost:4000/v1/reset-password', {
      otpBody: values.otp,
      email: values.email,
      ...
      if (res.status === 200) {
        axios.delete('http://localhost:4000/v1/clear').then((res) => {
          console.log(res);
        }).catch((err) => {
          console.log(err);
        });
